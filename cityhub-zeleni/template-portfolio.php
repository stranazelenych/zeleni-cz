...
if (have_posts()) : while (have_posts()) : the_post();
    ...
    <?php } elseif (!empty($html_title)) { ?>
        <div class="wrapper-featured-image">
            <div class="featured-image-container" style="background-color: <?php echo $wpGrade_Options->get('portfolio_header_bg_color'); ?>">
            ...
            <div class="main main-page">
                <div class="portfolio-container">
                    <div class="filter-container">
                        <h2 class="featuredworks-title">Naše úspěchy</h2>
                        <div class="filter-by-container">
                            <a href="#" class="filter-by-toggle"><span class="filter-by-text"><?php _e( 'Show All', wpGrade_txtd ); ?></span><i class="icon-chevron-down"></i></a>
                            <ul class="filter-by_list">
                                <li class="filter-by_list-item">
                                    <a href="<?php echo get_post_type_archive_link('portfolio') ?>" data-filter="*" title="<?php _e('View all projects',wpGrade_txtd) ?>"><?php _e('Show All', wpGrade_txtd); ?></a>
                                </li>
                                <?php $terms = get_terms(array('portfolio_cat'));
                                foreach ( $terms as $term ) { ?>
									<li class="filter-by_list-item <?php echo $term->slug; ?>">
                                        <a class="filter-by_link" data-filter="<?php echo '.' . $term->slug; ?>" href="<?php echo get_term_link($term) ?>" title="<?php sprintf( __( "View all projects in %s" ,wpGrade_txtd ), $term->name ); ?>"><?php echo $term->name ?></a>
                                    </li>
                                <?php }?>
                            </ul>
                        </div>
                    ...
                    <ul class="portfolio-items-list portfolio-items-list-full-width" id="portpage-portfolio-items-list">
                        ...
                        if ( $query->have_posts() ) : while ( $query->have_posts() ) : $query->the_post();
                            ...
                            <li class="portfolio-item <?php if(!has_post_thumbnail()) echo 'project-no-image '; 
                                                            if($categories) foreach($categories as $cat) { echo strtolower($cat->slug) . ' '; }?>">
